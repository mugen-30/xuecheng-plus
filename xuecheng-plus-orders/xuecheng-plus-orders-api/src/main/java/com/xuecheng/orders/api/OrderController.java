...
public class OrderController {
    ...
    @Resource
    IXcOrdersService iXcOrdersService;
    ...
    public void requestpay(@RequestBody AddOrderDto addOrderDto, HttpServletResponse httpResponse) throws IOException {
        //登录用户
        SecurityUtil.XcUser user = SecurityUtil.getUser();
        if(user == null){
            XueChengPlusException.cast("请登录后继续选课");
        }
        XcPayRecord payRecord = iXcOrdersService.createOrder(user.getId(), addOrderDto);
        //如果payNo不存在则提示重新发起支付
        if(payRecord == null){
            XueChengPlusException.cast("请重新点击支付");
        }
        //支付状态
...
//        alipayRequest.setReturnUrl("http://www.51xuecheng.cn/course/"+courseId+".html");//设置同步回调地址
        alipayRequest.setNotifyUrl("http://2j5dri.natappfree.cc/orders/paynotify");//在公共参数中设置回跳和通知地址
    ...
    public void receivenotify(HttpServletRequest request,HttpServletResponse out) throws UnsupportedEncodingException, AlipayApiException {
        ...
        if(verify_result) {//验证成功
            ...
            if (trade_status.equals("TRADE_SUCCESS")) {
                ...
                //处理逻辑。。。
                iXcPayRecordService.saveAliPayStatus(payStatusDto);
    ...
    @ResponseBody
    public PayRecordDto payresult() {
        String userId = SecurityUtil.getUser().getId();
        //调用支付宝接口查询
        return iXcPayRecordService.queryPayResult(userId);
    }
